...
void CVictoryState::Init()
{
	...
	// Load all the meshes
	MeshBuilder::GetInstance()->GenerateQuad("VICTORYSTATE_BKGROUND", Color(1, 1, 1), 1.f);
	MeshBuilder::GetInstance()->GetMesh("VICTORYSTATE_BKGROUND")->textureID = LoadTGA("Image//VictoryState.tga");
...
void CVictoryState::Exit()
{
	...
	EntityManager::GetInstance()->RemoveEntity(VictoryStateBackGround);

	// Remove the meshes which are specific to CVictoryState
	MeshBuilder::GetInstance()->RemoveMesh("VICTORYSTATE_BKGROUND");
